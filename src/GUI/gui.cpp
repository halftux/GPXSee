...
GUI::GUI()
{
...
#ifdef Q_WS_MAEMO_5
	_splitter->setStyleSheet("QSplitter::handle:vertical {height: 0px;}");
#endif
	...
	grabZoomKeys(true);
	connect(this, SIGNAL(s_zoom(QString)), _mapView, SLOT(zoom_maemo(QString)),Qt::UniqueConnection);
...
void GUI::ShowContextMenu(const QPoint &pos)
{
	...
	QMenu* coordmenu = contextMenu.addMenu( "coord ->" );
	QMenu* timemenu = contextMenu.addMenu( "time ->" );
	...
	coordmenu->addAction(_decimalDegreesAction);
	coordmenu->addAction(_degreesMinutesAction);
	coordmenu->addAction(_DMSAction);
...
void GUI::createMenus()
{
	...
	_stdMenu->addAction(_pathsAction);
...
void GUI::keys()
{
	...
	msgBox.setInformativeText(
	  QString("<div><table width=\"500\"><tr><td>") + tr("Next file")
	  ...
	  + tr("Digital zoom") + QString("</td><td><i>Shift+o/Shift+l</i></td></tr><tr><td>")
	  ...
	  + tr("Show Stats") + QString("</td><td><i>")
	  + _statisticsAction->shortcut().toString() + QString("</i></td></tr><tr><td>")
	  + tr("Toggle graph type") + "</td><td><i>"
	  + QKeySequence(TOGGLE_GRAPH_TYPE_KEY).toString() + "</i></td></tr><tr><td>"
	  + tr("Toggle time type") + "</td><td><i>"
	  + QKeySequence(TOGGLE_TIME_TYPE_KEY).toString() + QString("</i></td></tr><tr><td>")
...
#ifdef Q_WS_MAEMO_5
	msgBox.setText("<h3>" + tr("Directory of files") + "</h3>");
#else
...
void GUI::statistics()
{
	...
	QMessageBox msgBox(this);
	msgBox.setGeometry(30,30,640,450);
	msgBox.setWindowTitle(tr("Statistics"));
#ifdef Q_WS_MAEMO_5
	msgBox.setText("<h3>" + tr("Statistic Collection:") + "</h3>");
#else
...
{
	_mapView->setHidden(!show);
}

void GUI::showStatusbar(bool show)
{
	_mapView->setRescale(false);
...
void GUI::writeSettings()
{
	...
	if (!_files.isEmpty()) settings.setValue("lastfile",_files.at(0));
#endif
	settings.endGroup();
